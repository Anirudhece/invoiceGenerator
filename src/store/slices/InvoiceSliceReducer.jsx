...
const InvoiceSlice = createSlice({
  ...
  reducers: {
    checkCurrency: (state, action) => {
      ...
      if (invoiceFormId) {
        const invoiceElem = state.invoices.find(
          (ele) => ele.id.toString() === invoiceFormId
        );
    ...
    editFieldReducer: (state, action) => {
      ...
      if (invoiceFormId) {
        ...
        state.invoices[state.invoiceCount] =
          state.invoices[state.invoiceCount] || initialInvoice;
    ...
    calculateTotalReducer: (state, action) => {
      const { subTotal, taxAmount, discountAmount, total, invoiceFormId } =
        action.payload;
      if (invoiceFormId) {
        const invoiceElem = state.invoices.find(
          (ele) => ele.id.toString() === invoiceFormId
    ...
    updateInvoiceStateFromStorage: (state, action) => {
      return action.payload;// is same as state.push(action.payload) ?
    },
